...
- **Per-view TYPE enums** — verified across the 5-view LAB history family via cross-history TYPE survey (setup/test_result_history_probe.sql §54f-g, 30-day window). Each view writes its own non-overlapping TYPE vocabulary:
    - `V_P_LAB_TEST_RESULT_HISTORY`: `RMOD` / `DMOD` / `REVMOD` (this section)
    - `V_P_LAB_ACT_HISTORY`: `MODCOM` only (~3.6K/30d)
    - `V_P_LAB_TUBE_HISTORY`: `MODCOM` only (rare — 2 rows / 30d)
    - `V_P_LAB_PAT_HISTORY`: `MERGE` (patient-merge events, ~360/30d) / `MRNUPDATE` (~50/30d) / `REACTIVATE` (~1/30d)
    - `V_P_LAB_PLAB_HISTORY`: `TRANSFER` (stay-transfer events, ~62K/30d — by far the highest-volume history tag) / `MOVE` (~6/30d)
  The RMOD/DMOD/REVMOD vocabulary is specific to test result history. SCC support's "37-tag" event-vocabulary list crosses the LAB history family AND non-LAB modules (cancellation tables, call family, micro views, blood bank). Of those 37 tags, only **3 are actively written in this deployment** (RMOD/DMOD/MODCOM). The remaining 34 (TCANC/OCANC/SCANC/CALL/CALLED/MCALL/IMOD/IMP/MGEN/MADD/TGEN/SSITE/SRC_T/ANT_TD/ANT_TT/COM_INST/UNACT/REVU/FSTUDY/RLNO/RFL/FFIN/TAUDIT/etc.) **were not found** in: V_S_LAB_CANNED_MESSAGE.ID (§54a), V_P_LAB_MESSAGE (zero rows full-table — §52, §54d), V_P_LAB_CANCELLATION.CODE (vestigial column per dict), V_P_BB_Action/Test/Result.CODE (§55b, 30-day window). They likely live in views we haven't yet probed (V_P_BB_BB_Exception, V_P_BB_Patient_Message, V_P_BB_Nurse_Observation, V_P_LAB_CALL family, V_P_MIC_*) or are SCC documentation-only enum values not actively written in this deployment. There is **no single decoder/lookup table** that maps tag → description.
